...
4. `cd Standalone-app-v1 && python scripts/validate_dependencies.py` → passa (grafo task Python). Precondizione: `test -e scripts/validate_dependencies.py` (verificato presente e tracciato alla stesura; referenziato da `CLAUDE.md`); se un giorno mancasse, saltare **questo** step segnalandolo — non far fallire l'intera M6.
