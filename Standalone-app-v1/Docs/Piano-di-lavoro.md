...
> **Stato milestone (aggiornato 2026-07-03): PIANO COMPLETO — M0–M7 ✅ tutte completate e committate** — M1 `a82a3c0` · M2 `20d7f2e` · M3 `6c54bbd` · M4 `670cb21` · M5 `6e9241e`+`15be57c` · M6 `6cda1a3` · M7 `f1b83fb`. Il corpo sottostante è la copia storica del piano approvato (non riscritta); deviazioni motivate ed eccezioni permanenti (guard 58 non 56; eccezioni M6 step-1/step-3 per i 2 file di continuità) vivono in `Stato-avanzamento-pdl.md` §8. **`validate_dependencies.py`**, FAIL pre-esistente e fuori scope al momento di M6, è stato **risolto e committato in thread secondario** (`866a2b7`, 3 bug nello script, dati `AGENTS.md` intatti — ora VERDE; dettaglio in `Stato-avanzamento-pdl.md` §8). Follow-up fuori piano aperti: `Stato-avanzamento-pdl.md` §11.
