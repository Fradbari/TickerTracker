...
async def _get_finnhub_api_key() -> str:
    """Legge la chiave Finnhub salvata in AppConfig nel DB.
    Fallback: settings.FINNHUB_API_KEY (variabile d'ambiente)."""
    try:
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                select(AppConfig).where(AppConfig.key == "FINNHUB_API_KEY")
            )
        ...
        logger.warning(f"Impossibile leggere FINNHUB_API_KEY da DB: {e}. Uso fallback env.")

    # Fallback a variabile d'ambiente
    return settings.FINNHUB_API_KEY.get_secret_value() if settings.FINNHUB_API_KEY else ""
...
async def get_quote(symbol: str) -> dict | None:
    ...
    api_key = await _get_finnhub_api_key()
    if not api_key:
