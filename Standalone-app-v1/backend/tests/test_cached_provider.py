...
async def test_ttl_differentiation():
    ...
    historical = await cached.get_historical_prices("AAPL", date.today(), date.today())
    fundamentals = await cached.get_fundamentals("AAPL")
    ...
    historical2 = await cached.get_historical_prices("AAPL", date.today(), date.today())
    fundamentals2 = await cached.get_fundamentals("AAPL")
    ...
    historical3 = await cached.get_historical_prices("AAPL", date.today(), date.today())
    assert mock.call_count == 5  # New call for historical
    print("  ✓ After 2.5s: Historical expired (new call)")
    
    fundamentals3 = await cached.get_fundamentals("AAPL")
...
async def test_stale_data_fallback():
    ...
    print(f"  ✓ Returned stale cached data (is_stale=True)")
