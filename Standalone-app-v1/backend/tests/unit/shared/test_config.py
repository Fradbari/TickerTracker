...
class TestSecretStrFields:
    ...
    def test_secret_str_not_exposed_in_dict(self):
        ...
        assert dumped["DATABASE_URL"].get_secret_value() == "postgresql://user:password@localhost/db"
...
class TestMultiEnvironmentConfigurations:
    ...
    def test_production_environment_configuration(self):
        ...
        assert settings.DEBUG is False
        assert settings.LOG_LEVEL == "WARNING"
        assert settings.ENCRYPTION_KEY.get_secret_value() == "production-encryption-key-64-characters-long-hex-string"
...
class TestIntegrationScenarios:
    ...
    def test_complete_staging_setup(self, monkeypatch):
        ...
        monkeypatch.setenv("DATABASE_URL", "postgresql+asyncpg://staging_user:staging_pass@staging.db:5432/tt")
    ...
    def test_complete_production_setup_with_all_secrets(self, monkeypatch):
        ...
        monkeypatch.setenv("DEBUG", "false")
        monkeypatch.setenv("LOG_LEVEL", "WARNING")
        monkeypatch.setenv("DATABASE_URL", "postgresql+asyncpg://prod_user:prod_pass@prod.db:5432/tt")
