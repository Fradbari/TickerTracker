...
function Write-Task {
    param([string]$Text)
    Write-Host "[TASK] $Text" -ForegroundColor Yellow
}
    ...
    switch ($Command.ToLower()) {
        ...
        "up" {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ErrorMsg "Errore nell'avvio dei servizi"
                Pop-Location
                exit 1
            }
        }
        ...
        "down" {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ErrorMsg "Errore nell'arresto dei servizi"
            }
        }
        ...
        "health" {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ErrorMsg "PostgreSQL NOT ready"
            }
            
            # Check Redis
            Write-Host ""
            Write-Info "Redis:"
            docker exec tickertracker-redis redis-cli PING
            if ($LASTEXITCODE -eq 0) {
        ...
        "clean" {
            ...
            Write-ErrorMsg "ATTENZIONE: Questo comando rimuovera' TUTTI i dati persistenti!"
            Write-Host ""
        ...
        default {
            Write-ErrorMsg "Comando sconosciuto: $Command"
